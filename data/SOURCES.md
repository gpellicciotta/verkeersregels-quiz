...
| F34b1.svg | Wegwijzer: aanbevolen reisweg voor bepaalde categorieën van weggebruikers (volledige versie) | [File:Belgian road sign F34b1.svg](https://commons.wikimedia.org/wiki/File:Belgian_road_sign_F34b1.svg) |
| F34b2.svg | Wegwijzer: aanbevolen reisweg voor bepaalde categorieën van weggebruikers (vereenvoudigde versie) | [File:Belgian road sign F34b2.svg](https://commons.wikimedia.org/wiki/File:Belgian_road_sign_F34b2.svg) |
| F34c1.svg | Wegwijzer: aanbevolen reisweg naar een toeristische bestemming voor bepaalde categorieën van weggebruikers (volledige versie) | [File:Belgian road sign F34c1.svg](https://commons.wikimedia.org/wiki/File:Belgian_road_sign_F34c1.svg) |
| F34c2.svg | Wegwijzer: aanbevolen reisweg naar een toeristische bestemming voor bepaalde categorieën van weggebruikers (vereenvoudigde versie) | [File:Belgian road sign F34c2.svg](https://commons.wikimedia.org/wiki/File:Belgian_road_sign_F34c2.svg) |
...
- `rec-f34b1` / `rec-f34b2` / `rec-f34c1`: source images carry a Commons "SVG historic information road signs of Belgium" category; treated as a broad grouping label rather than a design-era flag (see the 2026-09-24 correction log entry below), but worth a second look if better sourcing turns up.
...
- 2026-09-24: task T0085 found four missing signs by extracting every sign code mentioned
  ...
  Wikipedia summaries: `F34b.1`, `F34b.2`, `F34c.1` and `F34c.2` (art. 71.2, "aanbevolen
  reisweg" route markers for cyclists/riders/pedestrians, one for a plain destination and
  one for a tourist destination). Added as `F34b1.svg`, `F34b2.svg`, `F34c1.svg` and
  `F34c2.svg`. Every other candidate gap surfaced by an initial Wikipedia pass (`A53`,
  `D1f`, `D3a`/`D3b`, `D9a`, `C48`/`C49`) was checked against the same consolidated PDF and
  turned out to be either a "KB-AR 03-06-2024" 2024-decree sign (out of scope per
  [[wegcode-cutover-2027]]), a discontinued code, or a PDF-layout artifact from a shared
  definition line, not a real current-law gap. The four added SVGs matched their Commons
  source by exact SHA-1 (verified individually via the Commons API, not the bulk
  `generate-sign-sources.py` run, which got rate-limited mid-run and was reverted rather
  than committed with 63 spurious "unresolved" regressions on previously-matched signs).
  `F34b1.svg`/`F34b2.svg`/`F34c1.svg` carry a Commons "SVG historic information road signs
  of Belgium" category alongside the matching "cycling/tourist route signs" categories;
  read as a broad umbrella grouping rather than a design-era flag, since none carry a
  "KB-AR 03-06-2024" tag and their SVG body uses the same blue wegwijzer-panel styling as
  `F34a.svg`, but flagged here for future manual review if that assumption turns out wrong.
