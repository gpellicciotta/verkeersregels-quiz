...
class TestNameQueryParam(unittest.TestCase):
    ...
    def test_js_defines_get_name_param(self) -> None:
        """Validates that the JS modules implement getNameParam helper with name, naam, and n aliases."""
        self.assertTrue(JS_DIR.exists(), "js directory must exist")
        js_code = _read_js()

        self.assertIn("function getNameParam()", js_code, "params.js must define getNameParam()")
    ...
    def test_start_quiz_uses_query_param_name(self) -> None:
        ...
        self.assertIn("function startQuiz()", js_code, "quiz.js must define startQuiz()")
    ...
    def test_restart_respects_name_query_param(self) -> None:
        ...
        self.assertIn("function restart()", js_code, "quiz.js must define restart()")
