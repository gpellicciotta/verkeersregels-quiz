...
class TestSignCarouselExplanation(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        with open(DATA_DIR / "translations.it.json", "r", encoding="utf-8") as f:
            cls.translations_it = json.load(f)
    ...
    def test_all_sign_questions_have_french_translations_for_sign_fields(self) -> None:
        """Validates that translations.fr.json includes signTitle and signExplanation for all sign questions."""
        sign_questions = [q for q in self.questions_data if "sign" in q]

        for q in sign_questions:
            qid = q["id"]
            self.assertIn(qid, self.translations_fr, f"Question {qid} missing in translations.fr.json")
            fr_item = self.translations_fr[qid]
            self.assertIn("signTitle", fr_item, f"Question {qid} missing 'signTitle' in translations.fr.json")
    ...
    def test_all_sign_questions_have_italian_translations_for_sign_fields(self) -> None:
        """Validates that translations.it.json includes signTitle and signExplanation for all sign questions."""
        sign_questions = [q for q in self.questions_data if "sign" in q]

        for q in sign_questions:
            qid = q["id"]
            self.assertIn(qid, self.translations_it, f"Question {qid} missing in translations.it.json")
            it_item = self.translations_it[qid]
            self.assertIn("signTitle", it_item, f"Question {qid} missing 'signTitle' in translations.it.json")
            self.assertIn("signExplanation", it_item, f"Question {qid} missing 'signExplanation' in translations.it.json")
            self.assertTrue(it_item["signTitle"].strip(), f"Question {qid} 'signTitle' empty in translations.it.json")
            self.assertTrue(it_item["signExplanation"].strip(), f"Question {qid} 'signExplanation' empty in translations.it.json")
